...
This document seeks to perform some exploratory analysis on [this dataset](https://d396qusza40orc.cloudfront.net/repdata%2Fdata%2FStormData.csv.bz2) of storm and weather events, published by the U.S. National Oceanic and Atmospheric Administration's (NOAA) storm database, in order to identify the most significant factors in terms of health and economic impact.
...
In terms of economic cost, Flood has by far the most signficant impact, then Hurricane/Typhoon, followed by Tornadao and Storm Surge.

The granularity of the breakdown of the data by weather event could have an impact on the results. Further analysis of this is recommended.
...
Given the low rate of data collection in the earlier years and the increased relevance (and presumably lower sampling error rate) of later data, I shall limit this analysis to data from 1995-2011.
```{r}
...
Group injuries and fatalities data by event type, summing up the number of each per event type. This is then combined into a single health impact dataset and order first by number of fatalities then injuries.
```{r}
...
Create a new variable `TOTALDMG` which is the sum of the `CROPDMGNUM` and `PROPDMGNUM` variables. Then group the data by event type, recording the sum of the `TOTALDMG` for each observation of that event type.
...
This figure demonstrates that the most costly weather event is Flood, followed by Hurricane/Typhoon, then Tornado and Storm Surge.

The results indicate that the granularity of weather event categorisation could result in some overlap, thus affecting the stated impact of certain weather events.
